<section id="main-content">
    <section class="wrapper">
        <div class="row state-overview"></div>

        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="mb-0">Edit BOM</h3>
                <a href="<?= site_url('bom') ?>" class="btn btn-secondary">Kembali</a>
            </div>

            <div class="card-body">
                <form method="post" action="<?= site_url('bom/update/' . $bom_l1['id']) ?>">
                    <!-- CSRF -->
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4">
                            ...
                            <select class="form-control" name="item_id" id="item_id" required>
                                ...
                                <?php foreach ($items as $it): ?>
                                    <option value="<?= $it['id'] ?>"
                                        <?= (int)$bom_l1['item_id'] === (int)$it['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($it['item_name'], ENT_QUOTES, 'UTF-8') ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" name="unit_id" id="unit_id" required>
                                ...
                                <?php foreach ($units as $u): ?>
                                    <?php $uName = $u['name'] ?? $u['unit_name'] ?? ''; ?>
                                    <option value="<?= $u['id'] ?>"
                                        <?= (int)$bom_l1['unit_id'] === (int)$u['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" name="brand_id" id="brand_id" required>
                                ...
                                <?php foreach ($brands as $b): ?>
                                    <?php $bName = $b['name'] ?? $b['brand_name'] ?? ''; ?>
                                    <option value="<?= $b['id'] ?>"
                                        <?= (int)$bom_l1['brand_id'] === (int)$b['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-12 mt-3">
                            ...
                            <input type="text" class="form-control" name="art_color" id="art_color"
                                value="<?= htmlspecialchars($bom_l1['art_color'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required>
                        </div>
                    </div>
                    ...
                    <div id="level2_items">
                        <table class="table table-bordered table-sm">
                            ...
                            <tbody id="tbodyLevel2">
                                <?php
                                // Render existing SFG rows (+ hidden inputs for materials)
                                $rowIndex = 0;
                                if (!empty($existing_l2)) :
                                    foreach ($existing_l2 as $row):
                                        $i = $rowIndex++;
                                        $sfgName = $row['item_name'] ?? '';
                                        $sfgId   = $row['item_id'] ?? null;
                                ?>
                                        <tr data-index="<?= $i ?>" data-sfg-id="<?= (int)$sfgId ?>">
                                            <td>
                                                <input type="text" class="form-control"
                                                    value="<?= htmlspecialchars($sfgName, ENT_QUOTES, 'UTF-8') ?>" readonly>
                                                <input type="hidden" name="level2_items[<?= $i ?>][item_id]" value="<?= (int)$sfgId ?>">
                                            </td>
                                            <td id="mat_summary_<?= $i ?>">
                                                <?php if (!empty($row['materials'])): ?>
                                                    <ul class="mb-0">
                                                        <?php foreach ($row['materials'] as $j => $m): ?>
                                                            <li>
                                                                <?= htmlspecialchars($m['item_name'] ?? ('ID:' . $m['item_id']), ENT_QUOTES, 'UTF-8') ?>
                                                                = <?= htmlspecialchars($m['consumption'], ENT_QUOTES, 'UTF-8') ?>
                                                            </li>
                                                        <?php endforeach; ?>
                                                    </ul>
                                                    <!-- Hidden inputs untuk materials existing -->
                                                    <div id="hidden_container_<?= $i ?>" style="display:none">
                                                        <?php foreach ($row['materials'] as $j => $m): ?>
                                                            <input type="hidden" name="level2_items[<?= $i ?>][materials][<?= $j ?>][item_id]"
                                                                value="<?= (int)$m['item_id'] ?>">
                                                            <input type="hidden" name="level2_items[<?= $i ?>][materials][<?= $j ?>][consumption]"
                                                                value="<?= htmlspecialchars($m['consumption'], ENT_QUOTES, 'UTF-8') ?>">
                                                        <?php endforeach; ?>
                                                    </div>
                                                <?php else: ?>
                                                    <em>Belum ada material.</em>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <button type="button" class="btn btn-primary btn-sm btnEditMaterials">Tambah / Edit Material</button>
                                                <button type="button" class="btn btn-danger btn-sm btnRemoveSFG ml-1">Hapus SFG</button>
                                            </td>
                                        </tr>
                                <?php
                                    endforeach;
...
<script>
    (function() {
        // ====== State ======
        let level2Index = (function() {
            // mulai dari jumlah baris existing
            const rows = document.querySelectorAll('#tbodyLevel2 tr');
            return rows.length;
        })();
      ...
      <table class="table table-sm table-bordered mb-2" id="tblLevel3">
        ...
        document.getElementById('btnAddSFG').addEventListener('click', function() {
            ...
            // Ambil 1 set SFG default (6 SFG dari brand tsb) untuk user pilih satu via prompt sederhana
            fetch('<?= site_url('bom/get_level2_by_brand') ?>/' + brandId)
                ...
                .then(data => {
                    ...
                    // Render pilihan sederhana (bisa ganti ke modal dropdown khusus jika mau)
        ...
        // ====== Delegations for Edit/Delete SFG ======
        tbodyLevel2.addEventListener('click', function(e) {
            ...
            if (target.classList.contains('btnEditMaterials')) {
                ...
                // Fetch kandidat material (SFG sesama brand + RAW)
                fetch('<?= site_url('bom/get_raw_or_sfg_for_material') ?>/' + sfgId)
                    ...
                    .then(payload => {
                        ...
                        // preload dari hidden (kalau ada)
                        const hidContainer = document.getElementById(`hidden_container_${i}`);
                        if (hidContainer) {
                            // Baca pasangan dari hidden, render ke modal
                            ...
                            if (itemInputs.length) {
                                ...
                                addMaterialRowToModal(); // baris kosong
                            }
                        } else {
                            addMaterialRowToModal(); // baris kosong
                        }
        ...
        // Simpan material dari modal → tanam hidden + ringkasan
        document.getElementById('btnSaveMaterials').addEventListener('click', function() {
            ...
            for (const r of rows) {
                ...
                // cari nama material utk ringkasan
        ...
        // (Opsional) Auto sinkron unit & brand dari item FG saat berubah
        itemSelect.addEventListener('change', function() {
            ...
            fetch('<?= site_url('bom/get_unit_and_brand_by_item') ?>/' + itemId)
                .then(r => r.json())
                .then(data => {
                    const unitId = data.units && data.units[0] ? data.units[0].id : '';
                    const brandId = data.brands && data.brands[0] ? data.brands[0].id : '';
                    if (unitId) document.getElementById('unit_id').value = unitId;
                    if (brandId) document.getElementById('brand_id').value = brandId;
                });
        });
